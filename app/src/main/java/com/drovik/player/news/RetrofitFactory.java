...
import com.drovik.player.BuildConfig;
import com.drovik.player.news.api.INewsApi;
...
public class RetrofitFactory {
    ...
    private static final Interceptor cacheControlInterceptor = new Interceptor() {
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            if (!NetworkUtil.isNetworkConnected(SailorCast.getContext())) {
    ...
    public static Retrofit getRetrofit() {
        synchronized (Object) {
            if (retrofit == null) {
                // 指定缓存路径,缓存大小 50Mb
                Cache cache = new Cache(new File(SailorCast.getContext().getCacheDir(), "HttpCache"),
                        1024 * 1024 * 50);

                // Cookie 持久化
                ClearableCookieJar cookieJar =
                        new PersistentCookieJar(new SetCookieCache(), new SharedPrefsCookiePersistor(SailorCast.getContext()));
