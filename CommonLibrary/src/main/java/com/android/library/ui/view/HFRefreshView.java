...
public class HFRefreshView extends ViewGroup implements OnDetectScrollListener{
    ...
    //圆形加载指示器最大下拉top
    private int rainbowMaxTop = 60;
    
    //圆形加载指示器松开刷新时的top
    private int rainbowStickyTop = 60;
    
    //圆形加载指示器初始top
    private int rainbowStartTop = -70;
    
    //圆形加载指示器的半径
    private int rainbowRadius = 55;
    
    private int rainbowTop = - 70;
    ...
    private final int MSG_START_REFRESH = 1;
    private final int MSG_STOP_LOAD_MORE = 2;
    ...
    public HFRefreshView(Context context) {
        this(context, null);
    }

    public HFRefreshView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public HFRefreshView(Context context, AttributeSet attrs, int defStyleAttr) {
                ...
                switch (msg.what) {
                    case MSG_STOP_REFRESH:
                        if (rainbowTop > rainbowStartTop) {
                            rainbowTop -= 5;
                            requestLayout();
                            mHandler.sendEmptyMessageDelayed(MSG_STOP_REFRESH, 15);
                        }
                    ...
                    case MSG_START_REFRESH:
                        ...
                        requestLayout();
                        mHandler.sendEmptyMessageDelayed(MSG_START_REFRESH, 15);
                        break;
                    case MSG_STOP_LOAD_MORE:
                    	if(mContentView.getFooterViewsCount()>0){
                        	mContentView.removeFooterView(mFooterView);
                      	}
                    	requestLayout();
    ...
    private void initDragHelper() {
        mDragHelper = ViewDragHelper.create(this, new ViewDragHelper.Callback() {
            ...
            public void onViewReleased(View releasedChild, float xvel, float yvel) {
                ...
                ViewCompat.postInvalidateOnAnimation(HFRefreshView.this);
    ...
    private boolean checkIsTop() {
        ...
        if (childCount > 0) {
            View firstChild = mContentView.getChildAt(0);
            if (firstChild.getTop() >= 0 && firstItem == 0 && currentTop == 0) {
                return true;
            } else {
            	return false;
            }
        ...
        mContentView.setOnScrollListener(new AbsListView.OnScrollListener() {
            ...
            public void onScroll(AbsListView view, int firstVisibleItem,
                ...
                if(lastItem == totalItemCount) {
                	View bottom = mContentView.getChildAt(mContentView.getChildCount() - 1);
                	if(bottom != null && mContentView.getBottom() == bottom.getBottom()){//scroll bottom
    ...
    public android.view.ViewGroup.LayoutParams generateLayoutParams(
            AttributeSet attrs) {
        return new HFRefreshView.LayoutParams(getContext(), attrs);
    }
    ...
    protected boolean checkLayoutParams(android.view.ViewGroup.LayoutParams p) {
        return p instanceof HFRefreshView.LayoutParams;
    }
    ...
    public void startRefresh() {
        if (!isRefreshing()) {
            mHandler.removeMessages(MSG_STOP_REFRESH);
    ...
    public void stopLoadMore(){
    	mHandler.removeMessages(MSG_STOP_LOAD_MORE);
        mHandler.sendEmptyMessage(MSG_STOP_LOAD_MORE);
    }
